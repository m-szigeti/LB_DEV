...
function getActiveLayerIdSet(enrichment, layerType) {
    if (Array.isArray(enrichment?.activeLayerIds) && enrichment.activeLayerIds.length) {
        return new Set(enrichment.activeLayerIds);
    }
    return new Set([SV_LAYER_TYPE_TO_ID[layerType]].filter(Boolean));
}
...
/**
 * One theme (or Overall only): spider of every theme score on this unit.
 * Several selected themes: only those themes, each in its own colour (not the full set).
 */
function getThemeSpiderModel(enrichment, layerType) {
    const allThemes = (enrichment?.themes || [])
        .map(normalizeSpiderTheme)
        .filter(Boolean);
    const activeLayerIds = getActiveLayerIdSet(enrichment, layerType);
    ...
    Array.from(activeLayerIds).forEach(layerId => {
        addSelected(themeById.get(layerId) || activeScores.find(score => score.layerId === layerId));
    });
    activeScores.forEach(score => addSelected(score));

    const stacked = selectedThemes.length > 1;
    const items = stacked ? selectedThemes : allThemes;
    return {
        stacked,
        items
    };
...
function generateThemeSpiderSection(model) {
    ...
    if (!model.stacked && model.items.length < 3) {
        return generateThemeBarsSection(model.items);
    }
    ...
    const legend = model.items
        .map(theme => `
            <div class="info-theme-spider-legend-item">
                ...
                <span class="info-theme-spider-legend-value">${escapeHtml(formatValue(theme.value))}</span>
            </div>
        `)
        .join('');
    ...
    return `
        <div class="info-section info-theme-section">
            <h4>${model.stacked ? 'Selected themes' : 'Theme scores'}</h4>
...
function drawThemeSpiderChart(canvas, model) {
    ...
    if (stacked) {
        ...
        [0.5, 1].forEach(ring => {
            const label = Math.abs(scale - 1) < 0.0001 ? String(ring) : formatValue(ring * scale);
